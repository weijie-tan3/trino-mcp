...
# When all slots are in use, additional calls are rejected immediately
# instead of queuing (to avoid the "stuck serialised calls" problem).
...
def _concurrency_limit_message() -> str:
    ...
    return (
        f"Error: All {config.max_concurrent_queries} concurrent query slot(s) are in use. "
